...
public class SlideImpl extends SceneObjectImpl implements ISlide {
	...
	public SlideImpl(String filePath, IPosition position, IRotation rotation, float scale, float ratio) {
		_imageURL = SlideImpl.class.getClassLoader().getResource(filePath);
		setPosition(position);
		setRotation(rotation);
		setScale(scale);
		this.ratio = ratio;

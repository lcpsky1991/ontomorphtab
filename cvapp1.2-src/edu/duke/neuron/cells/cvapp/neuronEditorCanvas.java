...
public class neuronEditorCanvas extends graphCanvas3 implements ActionListener {
	...
	String[] gotPointActionStrings = { " ", "click the point to trace from",
			"click ends of segment to cut",
			"click the two points to be joined", "click node(s) to be deleted",
			"click ends of segment in which to add node",
			"select ends of section", "select ends of section",
	...
	public void actionPerformed(ActionEvent e) {
		...
		/*
		 * Object source = e.getSource(); if (source instanceof MenuItem) {
		 * String sarg = ((MenuItem)source).getLabel(); sarg =
		 * e.getActionCommand(); if (gotMarks) { if (cell != null) {
		 * cell.markPointTypes (pmark, sarg); repaint(); } gotMarks = false; }
		 *  }
		 */
	}
		...
		if (lineList != null) {
			...
			if (view == SPROJECTION) {
				...
				if (imred != null && imgreen != null) {
					...
					gd.drawRedGreenLineList2(gr, gg, lineList, zCursor.value());
				...
				if (imred != null && imgreen != null) {
					...
					gd.drawAreaRedGreenLineList92(gr, gg, lineList, zCursor
							.value());
	...
	public boolean myMouseDrag(int x, int y) {
		...
		if (growMode && downPoint >= 0) {
			if (growpt == null) {
				...
				if (rightButton) {
					growpt[3] = Math.sqrt((dragOrig[0] - tcdp[0])
							* (dragOrig[0] - tcdp[0]) + (dragOrig[1] - tcdp[1])
							* (dragOrig[1] - tcdp[1]) + (dragOrig[2] - tcdp[2])
							* (dragOrig[2] - tcdp[2]));

...
class neulucData extends Object {
	...
	String[] sectionTypes = { "undefined", "soma", "axon", "dendrite",
			"apical_dendrite", "custom-1", "custom-2", "custom-n" };

	String[] headerField = { "ORIGINAL_SOURCE", "CREATURE", "REGION",
			"FIELD/LAYER", "TYPE", "CONTRIBUTOR", "REFERENCE", "RAW", "EXTRAS",
			"SOMA_AREA", "SHRINKAGE_CORRECTION", "VERSION_NUMBER",
			"VERSION_DATE", "*********************************************" };
	...
	public double[] getScale() {
		...
		while (!got && st.hasMoreTokens()) {
			...
			if ((n = s.indexOf("SCALE")) > 0) {
				got = true;
				StringTokenizer st1 = new StringTokenizer(s.substring(n + 5, s
						.length()));
				if (st1.countTokens() != 3) {
					System.out
							.println("cant parse shrinkage data in file header");
	...
	public void setScale(double[] scl) {
		...
		while (st.hasMoreTokens()) {
			...
			if (s.indexOf("SCALE") > 0) {
				sb.append("# SCALE " + mytrim(scl[0]) + " " + mytrim(scl[1])
						+ " " + mytrim(scl[2]) + "\n");
			} else {
				sb.append(s);
				sb.append("\n");
		...
		for (i = 0; i < ns; i++) {
			...
			if (ts.length() > 10 && ts.substring(0, 1).equals("[")) {
				...
				if (major == 0) {
					...
					// node of unknown radius - if so, set it to the current
					// radius
					...
					newPoint = new nlpoint(ptind, ptconv(minor), latestPoint,
							x, y, z, r);
					points.addElement(newPoint);
		...
		if (!done[p.myIndex]) {
			...
			if (p.nnbr == 1 && done[p.pnbr[0].myIndex]) {
				...
				sb3.append("}\n");
					...
					&& (done[p.pnbr[0].myIndex] != done[p.pnbr[1].myIndex])) {
				// inside a segment;
				pt3dappend(p);
	...
	public String HOCwriteNS() {
		...
		hocWriter hw = new hocWriter(points, sectionTypes);
		return hw.hocString();
	}
	...
	public String GENESISwriteHR() {
		...
		genesisWriter gw = new genesisWriter(points, sectionTypes);
	...
	public void clean() {
		...
		do {
			...
			tracePoint(rootpoint);
			removeFloating();
			System.out.println(" pass " + ipass + "  removed " + nremoved
	...
	public nlpoint havePointHere(double x, double y, double z) {
		...
		for (i = 0; i < np; i++) {
			...
			r = (x - q.x) * (x - q.x) + (y - q.y) * (y - q.y) + (z - q.z)
					* (z - q.z);
			if (r < rbest) {
				ibest = i;
				rbest = r;
		...
		if (lineList == null || !trustLineList) {
			...
			for (int i = 0; i < np; i++) {
				if (!done[i]) {
					...
					if (p.myIndex != i) {
						System.out.println("indexing mismatch " + i + " "
								+ p.myIndex);
					}
	...
	public void addPoint(double[] xyzr) {
		points.addElement(new nlpoint(points.size(), -1, xyzr[0], xyzr[1],
				xyzr[2], xyzr.length >= 4 ? xyzr[3] : 1.0));
		reindexPoints();
		trustLineList = false;
		trustPointList = false;
	}
	...
	public void addSegment(double[] xyzr1, double[] xyzr2) {
		nlpoint p1, p2;
		p1 = new nlpoint(points.size(), -1, xyzr1[0], xyzr1[1], xyzr1[2],
				xyzr1.length > 3 ? xyzr1[3] : 1.0);
		points.addElement(p1);
		p2 = new nlpoint(points.size(), -1, xyzr2[0], xyzr2[1], xyzr2[2],
	...
	public void addPoint(int ip0, int ip1) {
		...
		if (!got0 && !got1) {
			...
			pn = new nlpoint(points.size(), 1, 0.5 * (p0.x + p1.x),
					0.5 * (p0.y + p1.y), 0.5 * (p0.z + p1.z),
					0.5 * (p0.r + p1.r));
			points.addElement(pn);
			p0.addNeighbor(pn);
			p1.addNeighbor(pn);
	...
	public boolean recMarkSection(nlpoint p, boolean[] done, int mymark,
			int itarg) {
		boolean fnd = false;
		if (p.myIndex == itarg) {
			fnd = true;
		...
		if (points != null) {
			...
			for (int i = np - 1; i >= 0; i--) {
				...
				if (p.imark == 1) {
					...
					for (Enumeration e = points.elements(); e.hasMoreElements();) {
						boolean jnk = ((nlpoint) (e.nextElement()))
								.removeNeighbor(p);
					}
					points.removeElement(p);
				}
	...
	public void recMakeDG(nlpoint p, boolean[] done, int[] iny) {
		...
		for (int i = 0; i < p.nnbr; i++) {
			...
			if (!done[pd.myIndex]) {
				...
				lppd = Math.sqrt((p.x - pd.x) * (p.x - pd.x) + (p.y - pd.y)
						* (p.y - pd.y) + (p.z - pd.z) * (p.z - pd.z));

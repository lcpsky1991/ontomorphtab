...
class graphData3 extends Object {
	...
	public void setRangeConstraints(Double x0, Double y0, Double x1, Double y1) {
		if (xLowDef = (x0 != null))
			xLowLim = x0.doubleValue();
		if (yLowDef = (y0 != null))
			yLowLim = y0.doubleValue();
		if (xUpDef = (x1 != null))
			xUpLim = x1.doubleValue();
		if (yUpDef = (y1 != null))
			yUpLim = y1.doubleValue();
	...
	public void drawAxes(Graphics g, Color bgColor) {
		...
		if (xVisible) {
			...
			for (int i = (int) (0.5 + xLeft / dx); i <= (int) (xRight / dx); i++) {
				...
				g.drawLine(xx, height - bottomMargin, xx, height - bottomMargin
						+ 4);
		...
		if (yVisible) {
			...
			for (int i = (int) (0.5 + yBottom / dy); i <= (int) (yTop / dy); i++) {
				...
				xx = leftMargin + off;
				yy = height - bottomMargin
						- (int) (0.5 + dpdy * (i * dy - yBottom));
			...
			for (int i = (int) (yBottom / dy); i <= (int) (yTop / dy); i++) {
				...
				g.drawLine(leftMargin, height - yy, width - rightMargin, height
						- yy);
			}
	...
	public void drawOffsetText(Graphics g, double x, double y, int offx,
			int offy, String s) {
		int ix, iy;
		ix = leftMargin + (int) (dpdx * (x - xLeft));
		iy = height - (bottomMargin + (int) (dpdy * (y - yBottom)));
	...
	public void drawData(Graphics g, float[] x, float[] y, int[] rn) {
		...
		for (int i = 0; i < np; i++) {
			...
			iy[i] = height
					- (bottomMargin + (int) (dpdy * (y[rn[0] + i] - yBottom)));
			iy[i] = (iy[i] < 0 ? 0 : iy[i]);
			iy[i] = (iy[i] > height ? height : iy[i]);
	...
	public void fill3DRect(Graphics g, double x0, double y0, double x1,
			double y1, int idepth) {
		int ix, iy, w, h;
		ix = leftMargin + (int) (dpdx * (x0 - xLeft));
		iy = topMargin + (int) (dpdy * (yTop - y1));
		w = (int) (dpdx * (x1 - x0));
	...
	public void fillCenteredOval(Graphics g, double xc, double yc, int w, int h) {
		int ix, iy;
	...
	public void fillCenteredRect(Graphics g, double xc, double yc, int w, int h) {
		int ix, iy;
		ix = leftMargin + (int) (dpdx * (xc - xLeft));
		iy = topMargin + (int) (dpdy * (yTop - yc));
	...
	public void drawCenteredOval(Graphics g, double xc, double yc, int w, int h) {
		int ix, iy;
		ix = leftMargin + (int) (dpdx * (xc - xLeft));
		iy = topMargin + (int) (dpdy * (yTop - yc));
	...
	public void drawCenteredRect(Graphics g, double xc, double yc, int w, int h) {
		int ix, iy;
		ix = leftMargin + (int) (dpdx * (xc - xLeft));
		iy = topMargin + (int) (dpdy * (yTop - yc));
	...
	public void drawThickData(Graphics g, double[] x, double[] y, int np,
			int thickness) {
		int[] ix = new int[np];
		int[] iy = new int[np];
		int[] ixt = new int[np];
	...
	public final int xOfP(double[] p) {
		int x = x3c
				+ (int) (m3[0][0] * (xScale * p[0] - cen3[0]) + m3[0][1]
						* (yScale * p[1] - cen3[1]) + m3[0][2]
						* (zScale * p[2] - cen3[2]));
		return x;
	}
	...
	public final int yOfP(double[] p) {
		int y = y3c
				+ (int) (m3[1][0] * (xScale * p[0] - cen3[0]) + m3[1][1]
						* (yScale * p[1] - cen3[1]) + m3[1][2]
						* (zScale * p[2] - cen3[2]));
		return y;
	}
	...
	public final int zOfP(double[] p) {
		int z = z3c
				+ (int) (m3[2][0] * (xScale * p[0] - cen3[0]) + m3[2][1]
						* (yScale * p[1] - cen3[1]) + m3[2][2]
						* (zScale * p[2] - cen3[2]));
		return z;
	}
	...
	public void drawLineList(Graphics g, double[][][] linelist) {
		...
		for (ii = 0; ii < nl; ii++) {
			...
			boolean isvis = ((x0 > 0 && x0 < width && y0 > 0 && y0 < height) || (x1 > 0
	...
	public void drawAreaLineList9(Graphics g, double[][] lines, Color ccc) {
		int ii, i, j, k, nl;
		double c0, c1;
		double[] p0, p1;
		int[] xpol = new int[4];
		...
		for (ii = 0; ii < nl; ii++) {
			...
			if (isvis) {
				if (r < 0.75 * dpix) {
					...
					double dl = Math.sqrt((y1 - y0) * (y1 - y0) + (x1 - x0)
							* (x1 - x0));
					...
					/*
					 * int npw = (int)(r/dpix + 0.5); double dl = Math.sqrt
					 * ((y1-y0)*(y1-y0) + (x1-x0)*(x1-x0)); double vnx = -r/dpix *
					 * (y1 - y0) / dl; double vny = r/dpix * (x1 - x0) / dl;
					 * 
					 * xpol[0] = (int)(x0 - vnx); xpol[1] = (int)(x0 + vnx);
					 * xpol[2] = (int)(x1 + vnx); xpol[3] = (int)(x1 - vnx);
					 * 
					 * ypol[0] = height - (int)(y0 - vny); ypol[1] = height -
					 * (int)(y0 + vny); ypol[2] = height - (int)(y1 + vny);
	...
	public void drawRadiusLineList9(Graphics g, double[][] lines, Color cca) {
		int ii, i, j, k, nl;
		double c0, c1;
		double[] p0, p1;
		int[] xpol = new int[4];
		...
		for (ii = 0; ii < nl; ii++) {
			...
			if (isvis) {
				...
				g.drawLine(x0 - vnx0, height - y0 + vny0, x0 + vnx0, height
						- y0 - vny0);

				g.drawLine(x1 - vnx1, height - y1 + vny1, x1 + vnx1, height
						- y1 - vny1);

				g.drawLine(x0 - vnx0, height - y0 + vny0, x1 - vnx1, height
						- y1 + vny1);
	...
	public void drawBlockLineList(Graphics g, double[][] lines) {
		...
		for (ii = 0; ii < nl; ii++) {
			...
			g.drawLine(x0 - vnx0, height - y0 + vny0, x0 + vnx0, height - y0
					- vny0);

			g.drawLine(x1 - vnx1, height - y1 + vny1, x1 + vnx1, height - y1
					- vny1);

			g.drawLine(x0 - vnx0, height - y0 + vny0, x1 - vnx1, height - y1
					+ vny1);
	...
	public void drawRedGreenLineList9(Graphics g, double[][] lines, double zrg) {
	...
	public void drawRedGreenLineList2(Graphics g1, Graphics g2,
			double[][] lines, double zrg) {
		int ii, i, j, k, nl, x0, y0, z0, x1, y1, z1, dz0, dz1;
		double c0, c1;
		double[] p0, p1;
		p0 = new double[3];
	...
	public void drawAreaRedGreenLineList92(Graphics g1, Graphics g2,
			double[][] lines, double zrg) {
		int ii, i, j, k, nl, x0, y0, z0, x1, y1, z1, dz0, dz1;
		double c0, c1;
		double[] p0, p1;
	...
	public void rotateXY(double rx, double ry) {
		...
		m3[0][0] = cy * m3sav[0][0] - sx * sy * m3sav[1][0] + cx * sy
				* m3sav[2][0];
		m3[0][1] = cy * m3sav[0][1] - sx * sy * m3sav[1][1] + cx * sy
				* m3sav[2][1];
		m3[0][2] = cy * m3sav[0][2] - sx * sy * m3sav[1][2] + cx * sy
				* m3sav[2][2];
		...
		m3[2][0] = -sy * m3sav[0][0] - sx * cy * m3sav[1][0] + cx * cy
				* m3sav[2][0];
		m3[2][1] = -sy * m3sav[0][1] - sx * cy * m3sav[1][1] + cx * cy
				* m3sav[2][1];
		m3[2][2] = -sy * m3sav[0][2] - sx * cy * m3sav[1][2] + cx * cy
				* m3sav[2][2];

	}

	public void axisRotate(double thax, double thr, double[][] m3a,
			double[][] m3b) {
		...
		for (int i = 0; i < 3; i++) {
			m3b[0][i] = (cf * cf + sf * cr * sf) * m3a[0][i]
					+ (cf * sf - sf * cr * cf) * m3a[1][i] + (-sr * sf)
					* m3a[2][i];

			m3b[1][i] = (sf * cf - cf * cr * sf) * m3a[0][i]
					+ (sf * sf + cf * cr * cf) * m3a[1][i] + (cf * sr)
					* m3a[2][i];

			m3b[2][i] = (sr * sf) * m3a[0][i] + (-sr * cf) * m3a[1][i] + (cr)
					* m3a[2][i];
		}
	}
	...
	public void zRotate(double theta, double[][] m3a, double[][] m3b) {
		...
		for (int i = 0; i < 3; i++) {
			m3b[0][i] = (cf) * m3a[0][i] + (sf) * m3a[1][i];

			m3b[1][i] = (-sf) * m3a[0][i] + (cf) * m3a[1][i];
	...
	public void dragRotate(int xdown, int ydown, int x, int y, boolean mode1) {
		/*
		 * if (!initialisedRotation) rotate3init (xdown, ydown); double ry =
		 * (3.0 * (x - xdown)) / width; double rx = (3.0 * (y - ydown)) /
		 * height;
		 */
		double dx, dy;
		double dd = 1. / width;
		...
		if (mode1) {
			...
			zRotate(-Math.atan2((double) (xdown - xcen),
					(double) (ydown - ycen)), m3sav, m3t);
			zRotate(Math.atan2(dx, dy), m3t, m3);

		}
	}

	public void panZoom(int xdown, int ydown, int x, int y, boolean longClick) {
	...
	public void fixZoom(int x, int y, double f) {
		...
		if (y > height - topMargin)
			tickGridx = tickGridx + (f < 1. ? -1 : 1);
		if (x > width - rightMargin)
			tickGridy = tickGridy + (f < 1. ? -1 : 1);
	}
	...
	public double[] rotInv(double[] p) {
		...
		det = m3[0][0] * (m3[1][1] * m3[2][2] - m3[2][1] * m3[1][2]) - m3[0][1]
				* (m3[1][0] * m3[2][2] - m3[2][0] * m3[1][2]) + m3[0][2]
				* (m3[1][0] * m3[2][1] - m3[2][0] * m3[1][1]);

		r[0] = (m3[0][1] * (m3[1][2] * p[2] - m3[2][2] * p[1]) - m3[0][2]
				* (m3[1][1] * p[2] - m3[2][1] * p[1]) + p[0]
				* (m3[1][1] * m3[2][2] - m3[2][1] * m3[1][2]))
				/ det;

		r[1] = -(m3[0][0] * (m3[1][2] * p[2] - m3[2][2] * p[1]) - m3[0][2]
				* (m3[1][0] * p[2] - m3[2][0] * p[1]) + p[0]
				* (m3[1][0] * m3[2][2] - m3[2][0] * m3[1][2]))
				/ det;

		r[2] = (m3[0][0] * (m3[1][1] * p[2] - m3[2][1] * p[1]) - m3[0][1]
				* (m3[1][0] * p[2] - m3[2][0] * p[1]) + p[0]
				* (m3[1][0] * m3[2][1] - m3[2][0] * m3[1][1]))
				/ det;
		...
		double ddif = 0.0;
		for (int i = 0; i < 3; i++)
			ddif += (p[i] - rp[i]) * (p[i] - rp[i]);

		if (ddif > 0.0001) {
			System.out.println("Error inv arg: " + p[0] + " " + p[1] + " "
					+ p[2]);
			System.out.println("inv res: " + r[0] + " " + r[1] + " " + r[2]);
			System.out.println("inv m*res: " + rp[0] + " " + rp[1] + " "
					+ rp[2]);
		}
		return r;
	}
	...
	public double[] worldToPixel3(double[] world) {
		...
		p3[1] = yOfP(world);
		p3[2] = zOfP(world);
		// System.out.println (" wtp3 " + world[0] +" " +world[1] + " "
		// +world[2]);
	...
	public double[] pixel3ToWorld(double[] pix3) {
		...
		// System.out.println (" p3tw " + pix3[0] + " " +pix3[1] + " "
		// +pix3[2]);
...
class edgeCube extends Object {
	...
	public void offsetScaledDraw(Graphics g, graphData3 gd, double dy,
			double[] cen3) {

...
class rescalableFloatSlider extends Canvas implements MouseListener,
	...
	rescalableFloatSlider(int loglin, String name, doublePointer c, double a,
			double b) {
		integ = false;
		dPointer = c;
		v0 = a;
	...
	rescalableFloatSlider(int loglin, String name, integerPointer c, int a,
			int b) {
		integ = true;
		iPointer = c;
		log = (loglin == LOG);
	...
	public void setZeroable(boolean b) {
		if (integ) {
			...
			iszero = (zeroable ? iPointer.value() < (int) (Math.pow(10.0, v0))
					: false);
			wzb = (zeroable ? 26 : 0);
			if (!zeroable)
				iszero = false;
			if (iszero && iPointer != null)
				iPointer.set(0);
		} else {
			zeroable = (log ? b : false);
	...
	public void drawUpButton(Graphics g, Color c, int x, int y, int w, int h) {
		g.setColor(c.darker());
	...
	public void drawDownButton(Graphics g, Color c, int x, int y, int w, int h) {
	...
	public void realPaint(Graphics g) {
		...
		xslider = (int) (wzb + 15 + (width - wzb - wpb - 30) * (linValue - v0)
				/ (v1 - v0));
	...
	public void checkRange(boolean lim, boolean val) {
		if (lim) {
			if (havelimits[0] && v0 < limits[0])
				v0 = limits[0];
			if (havelimits[1] && v1 > limits[1])
				v1 = limits[1];
		}
		if (val) {
			if (linValue < v0)

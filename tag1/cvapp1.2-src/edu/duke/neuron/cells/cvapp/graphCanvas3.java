...
public class graphCanvas3 extends Canvas implements MouseListener,
	...
	public void makeEchoArray() {
		...
		if (echoArray == null || echoArray.length < nx
				|| echoArray[0].length < ny)
			echoArray = new int[nx][ny];
		for (i = 0; i < nx; i++) {
			for (j = 0; j < ny; j++)
				echoArray[i][j] = -1;
		}
	...
	public void getNPoints(int np) {
		...
		if (pointList == null) {
			...
			/*
			 * } else if (echoMode) { echoMode = false; pointsGot = 0;
			 * pointsNeeded = 0;
			 *  } else { echoMode = true; makeEchoArray (); pointsNeeded = np;
			 * pointsGot = 0;
			 *  }
			 */
		} else {
	...
	public void mousePressed(MouseEvent e) {
		 ...
		 * e.BUTTON2_MASK + " " + e.BUTTON3_MASK + " " + e.ALT_MASK + " " +
		 * e.CTRL_MASK);
		 */
	...
	public void gcMouseDown(int x, int y, long when, int button) {
		...
		if (echoMode && echoPoint >= 0 && pointGetting) {
			...
			for (int i = 0; i < pointsGot; i++)
				ncp[i] = chosenPoints[i];
			chosenPoints = ncp;
	...
	public void gcMouseDrag(int x, int y) {
		...
		if (downPoint >= 0) {
			...
			if ((x - tzx1) * (x - tzx1) + (y - tzy1) * (y - tzy1) > 64) {
				double a = (tzx1 - tzx0) * (y - tzy0) - (tzy1 - tzy0)
						* (x - tzx0);
				a = a
						/ ((tzx1 - tzx0) * (tzx1 - tzx0) + (tzy1 - tzy0)
								* (tzy1 - tzy0));
	...
	/*
	 * public boolean myKeyDown (char key) { return true; }
	 */

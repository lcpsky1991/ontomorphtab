...
public class NormalGenerator {
	 ...
	 * vertices. Every 3 consecutive vertices define one triangle.
	 * @param vertices The vertex coordinates. Every three values define one 
	 * vertex
	 * @param indices An array containing int values. Each value belongs to one 
	 * vertex in the <code>vertices</code> array, the values are stored in the 
	 * same order as the vertices. For equal vertices in the 
	 * <code>vertices</code> array, the indices are also equal.
	 * @param creaseAngle The maximum angle in radians between faces to which 
	 * normals between the faces are interpolated to create a smooth transition 
	 * @return
	 */
